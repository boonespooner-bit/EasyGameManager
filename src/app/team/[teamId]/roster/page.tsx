      ...
      <div className="mt-8">
        ...
        <div className="grid gap-2">
          {team.members.map((m) => {
            ...
            const isCurrentUserHeadCoach = team.members.some(
              (mem) => mem.user.id === currentUserId && mem.role === "head_coach",
            );
            const canManage = isCurrentUserHeadCoach && m.role !== "head_coach";
            const hasEditAccess = m.role === "assistant_coach";
            return (
              <div key={m.id} className="bg-white rounded-lg border p-3 flex items-center justify-between">
                ...
                {canManage && (
                  <div className="flex items-center gap-3">
                    <label className="flex items-center gap-1.5 cursor-pointer" title={hasEditAccess ? "Revoke edit access" : "Grant edit access"}>
                      <span className={`text-xs ${hasEditAccess ? "text-green-600" : "text-gray-400"}`}>
                        {hasEditAccess ? "Can edit" : "View only"}
                      </span>
                      <button
                        onClick={async () => {
                          const newRole = hasEditAccess ? "viewer" : "assistant_coach";
                          await fetch(`/api/teams/${teamId}/coaches`, {
                            method: "PATCH",
                            headers: { "Content-Type": "application/json" },
                        ...
                        className={`relative inline-flex h-5 w-9 items-center rounded-full transition-colors ${
                          hasEditAccess ? "bg-green-500" : "bg-gray-300"
                        }`}
                      >
                        <span
                          className={`inline-block h-3.5 w-3.5 rounded-full bg-white transition-transform ${
                            hasEditAccess ? "translate-x-[18px]" : "translate-x-[3px]"
                          }`}
                        />
                      </button>
                    </label>
                    {confirmRemoveCoachId === m.id ? (
                      <div className="flex items-center gap-2">
                        ...
                        <button
                          onClick={() => setConfirmRemoveCoachId(null)}
                          className="text-gray-500 hover:text-gray-700 text-xs font-medium px-2 py-0.5 rounded border border-gray-300"
                        >
                          No
                        </button>
                      ...
                      <button
                        onClick={() => setConfirmRemoveCoachId(m.id)}
                        className="text-red-400 hover:text-red-600 transition-colors"
                        title="Remove coach"
                      >
                        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" className="w-4 h-4">
